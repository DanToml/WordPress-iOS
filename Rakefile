...
def xcodebuild(*build_cmds)
  ...
  cmd += " | bundle exec xcpretty -f `bundle exec xcpretty-travis-formatter` -r junit --output $CIRCLE_TEST_REPORTS/xcode/test.xml && exit ${PIPESTATUS[0]}" unless ENV['verbose']
  sh(cmd)
end
